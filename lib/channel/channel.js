...
Channel.prototype = {
  ...
  hdl:{
    connect: function(){
      trace && console.log('channel.hdl.connect')
      ...
      trace && console.log('channel.hdl.end')
      if(!this._errorFired){
    ...
    close: function(){
      trace && console.log('channel.hdl.close')
      if(!this._errorFired){
        this._errorFired = true
    ...
    error: function(err){
      trace && console.log('channel.hdl.error', err)
      var errno0 = errno[err.errno] || errno.EBADF
      this._errorFired = true
    ...
    message: function(m){
      trace && console.log('channel.hdl.message', m)
      if(!(typeof m === 'object' && m.length === 3)){
        trace && console.log('RPC message is not a tuple', m)
        this._hdl.error({errno: errno.EBADMSG})
        return false
      }
      ...
      if(!(typeof mid === 'number'
        ...
        trace && console.log('bad RPC message tuple')
        this._hdl.error({errno: errno.EBADMSG})
        return false
      }
      ...
      switch(m[0]){
        ...
        case RPC.error: {
          var errno = args[0], message = args[1]
        ...
        default: {
          trace && console.log('discarding a message of unknown type', m)
        }
      }
      return true
    },
    ...
    data: function(buf){
      ...
      while(m = unpackMessage(this._inBuffer)){
        ...
        trace && console.log('channel got message', m)
        trace && console.log('unpackMessage.bytesParsed', unpackMessage.bytesParsed)
