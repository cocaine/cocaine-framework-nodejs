...
function unpackMessage(buf){
  ...
  if(!(buf[0] === 0x93)){
    __assert(false , 'buf[0] === 0x93 ' + buf.toString())
  }
  ...
  if(method === RPC.chunk){
    ...
    if(!buf[parsed] === 0x91){
      __assert(false, 'buf[parsed] === 0x91 '+buf.slice(parsed).toString())
    }
    parsed += 1
...
function parseInt(b,i){
  ...
  if((0x7f & b0) === b0){
    ...
    case 0xd3: //int64
      ...
      break
    default:
      throw new Error('parse int failed: '+hexy.hexy(b.slice(i)))
    }
...
function parseBuffer(b,i){
  ...
  default:
    throw new Error('parse binary buffer failed: '+hexy.hexy(b.slice(o)))
  }
  return val
}
