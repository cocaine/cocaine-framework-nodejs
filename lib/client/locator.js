...
function _normalizeEndpoints(endpoints0, defaultEndpoints){
  ...
  var message =[
    ...
    '    where Host is one of ipv4, ipv6, fqdn',
    'or should be string in form of',
    '  endpoint1,endpoint2,endpoint3'].join('\n')

  function _normalizeEndpointsList(endpoints0){
    var endpoints
    try {
      endpoints = endpoints0.map(_normalizeEndpoint)
    } catch (e){
      e.message += '\n' + fmt(message, endpoints0)
      throw e
    }
    return endpoints
  }
  ...
  if(typeof endpoints0 === 'string') {
    if(endpoints0.indexOf(',') !== -1) {
      endpoints = endpoints0.split(',').map(function(e){return e.trim()}).filter(function(e){return e})
      endpoints = _normalizeEndpointsList(endpoints)
    } else {
    ...
    if(endpoints0.length === 2 && typeof endpoints0[1] === 'number'){
      ...
      endpoints = _normalizeEndpointsList(endpoints0)
    }
  } else {
...
module.exports._normalizeEndpoints = _normalizeEndpoints
