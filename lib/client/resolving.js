...
function Session(serviceName, methodName, methodId, args, cb, unpacker){
...
Session.prototype = {
  ...
  pushChoke: function(){
    ...
    // XXX: actually, why would we unpack concatenated chunks as a whole?
    var result = unpacker(Buffer.concat(this._chunks))
    this._cb(null, result)
  },
  pushError: function(code, message){
    debug('pushError', code, message)
    __assert(!this._done)
    this._done = true
...
Resolving.prototype = {
  ...
  close: function(e){
    debug('.close(%s), state', e, this._service._state)
    if(this._resolving){
      this._resolving = false
  ...
  call: function(methodName, args, cb){
    if(this.__methods && typeof this.__methods[methodName] === 'number'){
    ...
    if(this.options.methods){
      var unpacker = this.options.methods[methodName] || mp.unpack
    } else {
      var unpacker = mp.unpack
    }
    var s = new Session(this.__name, methodName, methodId, args, cb, unpacker)
    this.callSession(s)
  },
  ...
  handlers: {
    ...
    serviceBeforeError: function(err) {
      ...
      debug('serviceBeforeError', this.__name, err)
      debug(new Error('from:').stack)
      if(state === 'connected'){
        debug("state === 'connected'")

        if(this._reconnects++ < this.options.maxReconnects) {
          debug('if policy permits, try reconnect')
          
          if(this.__stateless){
            debug('if policy permits, rescue sessions')
            this._hdl.rescueAllRequests()
          }

          if(this.options.trySameEndpoint) {
            debug('if policy permits, try the same endpoint')
        ...
        if(this._connectTries < this.options.maxConnectTries) {
          debug('if policy permits, reconnect with resolve')
          this._scheduleConnect()
          this._service._setErrorHandler(['close', err.errno])
          return true
        }
