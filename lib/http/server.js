...
HTTPParser.prototype={
  execute:function(chunk){
    if(!this._env){
      ...
      console.log("==== js: got header chunk")
      try {
      ...
      if(this._has_body = METHOD_HAS_BODY[method]){
        this._body_pending =
          this._body_length = parseInt(env.meta.headers["CONTENT-LENGTH"])
      }
      this._env = env
      try {
      ...
      console.log("==== js: got body chunk",chunk.length)
