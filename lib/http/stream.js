...
var Connection=function(streamHandle){
  ...
  this._request_has_body=false
  this._request_content_length=0
}
...
function onread(chunk){
  ...
  if(!self._info){
    self._info=mp.unpack(chunk)
    if(self._info.REQUEST_METHOD in method_has_body){
      self._data_pending=self._info.HTTP_CONTENT_LENGTH
      self._request_has_body=true}}
  else if(self._request_has_body){
    __assert(chunk.length <= self._data_pending)
    self._data_pending -= chunk.length
    self.onBody(chunk)}
  else {
    __assert(0,"request with no body has pending data")}
  
  if(self._data_pending === 0){
    self.onMessageComplete && self.onMessageComplete()}
