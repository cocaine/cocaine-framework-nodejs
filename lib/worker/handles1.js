...
var debug = require('debug')('co:handles1')
...
function StreamHandle(id,worker,listenHandle){
  ...
  this._cocaine_lingering_shutdown_enabled = false
...
StreamHandle.prototype = {
  ...
  close:function(cb){
    ...
    if(this._cocaine_lingering_shutdown_enabled) {
      this._worker._handle.send(mp.pack([this._id, 0, [""]]))
    } else {
  ...
  writeBinaryString:function(s){
    return this.writeString(s, 'binary')
  },
  ...
  setCocaineLingeringShutdown: function(v){
    __assert(v === true, "setCocaineLingeringShutdownworks in enable-only fashion, "+
             "so use it as setCocaineLingeringShutdownworks(true)")
    this._cocaine_lingering_shutdown_enabled = true
  },

  cocaineLingeringShutdownClose: function(){
    this._worker._handle.send(mp.pack([this._id, 2, []]))
  },
